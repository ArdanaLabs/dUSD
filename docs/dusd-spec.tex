...
The tests must check that the policy
  - only validates if the Configuration UTXO is present along with the Configuration NFT
  - only validates when a policy at a specific index also validates
    - we might choose to have special indexes with reserved purposes
    - e.g. index 0 is for Configuration Management, index 1 is for dUSD minting logic delegation
  - dUSD can only be minted when the referenced policy mints 1 token
  - the amount of dUSD minted must be legitimate according to system parameters and vault state
