...
% V2: This purpose will be realized when the Peg Stabilization Module (PPM) is implemented
%
...
% V2: Minting dUSD in the buffer will be part of the PSM
%
...
% V2: Automated price-corrective action will be support in the PSM
%
...
% V2: For usage in algorithmic price stabilization
%
...
% V2: For performing algorithmic price stabilization
% 
...
\begin{itemize}
  % V2: Part of the Peg stabilization module
  %
...
%   \item Only admin can set stabilization interval
%   \item Only admin can set liquidityFactor
%   \item Only admin can set orderLogSize
% \end{itemize}

% V2: For algorithmic price stabilization
% 
% Peg Stabilization Bot
% \begin{itemize}
%   \item The process will run at the rate set by the stabilization interval, with
%    the default value of 1 hour.
%   \item The process will close any existing order from the previous run.
%   \item The process will push the previous order's quantity, status, and the dUSD price and circulating supply at time it was created to the log.
...
%    back to peg. The quantity should equal the following: escalationScalar * liquidityFactor * supply * (price - 1) where escalationScalar is calculated with the following logic: if the current price and the previous price are both outside of the peg bounds and the current price has deviated further than the previous price, calculate the percentage at which it deviated, and add 1 to it, and use that number to set the escalationScalar. Else, escalationScalar is 1.
...
% V2: Auction sizing algorithm will be implemented in PSM
%
% When auctions are triggered, the auction sizing algorithm determines how much
% dUSD/ADA is sold.


% V2: Reactive price correction will be implemented in the PSM 
% 
% Whenever dUSD is trading above peg, we are selling dUSD at a
% discount, and whenever dUSD is trading below peg, we are selling ADA at a
% discount.
% This methodology also prevents us from over-correcting by launching auctions
% which are too big. \\
...
% V2: Will be implemented for testing algorithmic price stabilization
%
