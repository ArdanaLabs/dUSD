...
Any user can detect that a vault is in an unhealthy state, and buy \todo{part of the
collateral} (user is liquidated in their entirety, collateral is sent auction model which 
parses it out into lots which users can buy)

in the vault at an admin-set discount rate compared to the market
...
\todo{Point above is wrong. The CR would drop in this scenario...}
...
\todo{By burning 10 percent of the paid back debt, that will reduce the capital efficiency of the liquidation process. while it is true that the excess supply must be offset. it is being offset using the surplus auctions, thus essentially canceling out the excess supply}
...
\todo{no dusd price module, no automatic debt auction}
...
\todo{not burning stability fee but directing to surplus buffer.}
\end{itemize}
