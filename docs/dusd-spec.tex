...
\subsection{Set Protocol Configuration}

The protocol logic can be set by an authorised party by changing the Configuration datum.
...
Uniqueness of the NFT is guaranteed in the usual way by parameterising the minting policy for the NFT by a TxOutRef which must be spent.

The tests must check that the policy
\begin{itemize}
  \item only validates if the TxOut it expects is being spent.
	% We've decided not to enforce this with the minting policy
	% because once the nft is minted anyone can verify that this was obeyed
...
The Main Script Redeemer is an index into this list and ensures that exactly one of the referenced token is minted.
