...
The config utxo consists of two onchain components an NFT and an address validator.

\subsubsection{NFT}

This can be the same NFT from hello discovery and can have the same tests.
CTL recently added balancing constraints that we can  use to improve these tests,
so we may want to take advantage of that.


\subsubsection{Address validator}

The address validator needs to ensure that the datum retains the NFT
and that edits are prepend only. The exact format of the update entries is
TBD (and probably needs to be pretty consistent for the sake of the DUSD minting policy)
but shouldn't actually change how this validator works.

A valid update keeps the NFT with the utxo
and adds prepends an arbitrary string to the list
in the datum.


To test this validator we can:
\begin{itemize}
	\item Validate a Tx which is a valid update
	\item Fail to validate an update where the NFT is removed
	\item Fail to validate an update where the first entry is also changed
\end{itemize}
