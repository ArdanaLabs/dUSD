...
When doing so, I have the choice between three vault types (ADA-A, ADA-B and
ADA-C), which have progressively higher liquidation ratios but lower stability
fees.
...
There is a minimum size for dUSD loans, called the ``minimum loan
size''\footnote{
...
will be charged a liquidation penalty for allowing his vault to become
unhealthy.
The liquidation penalty is initially set to $10\%$.
...
\subsubsection{Initialize buffer}

Anyone can set up a buffer, but has to pass along three admin keys.
The entire system will keep those keys as parameters, to ensure remaining in
sync.
In addition, initializing the buffer requires setting a list of accepted
collateral types (each with a price module), list of accepted vault types, and a
liquidation ratio for each vault type.
...
The admin user can set the stability fees\footnote{
  When setting a new stability fee, only the fee for that vault type is updated.
} (in percentage per year), through running a `curl' command on the Ardana
Tenant.

\subsubsection{Set minimum loan size}

The admin user can set the minimum loan size (in dUSD), through running a `curl'
command on the Ardana Tenant.
...
\begin{itemize}
  ...
  \item Users cannot take out a loan or pay one back that leaves the dUSD debt
    above $0$ but below the minimum loan size\footnote{
      The puprose of the minimum loan size is to ensure good debt is issued
      against a vault to incentivize a liquidator to liquidate it should it
...
% simply create multiple vaults.
  ...
  \item $10\%$ of the dUSD paid by the liquidator, is burned rather than used to
    pay back the debt. This liquidation penalty is meant to disincentivize users
...
\begin{itemize}
  ...
  \item Admin and only admin can set the protocol-wide parameters (stability
    fee, liquidation discount, liquidation penalty and minimum loan size)
...
\begin{itemize}
  ...
  \item Liquidation is only allowed if the current price and the price of an
    hour ago\footnote{
      If the price module only offers one price, the protocol will be considered
      on hold, i.e. no new loans and no liquidations.
      This scenario only happens when the price module was just created.
      In practice, we will launch the price module (and buffer) at least an hour
      before we launch the website that makes it easy for people to set up
      vaults.
    }
    both state that the current collateralization ratio is below the
    liquidation ratio.
...
\begin{itemize}
  \item Test (separately) what happens in case of datum overflow for the vault,
    buffer or price module
  ...
  \item In version 2, should the price oracles and OSM be 1 UTXO or separate
    ones?
\end{itemize}
...
\begin{itemize}
  ...
  \item Liquidation penalty upon initialization
  \item Liquidation discount upon initialization
  \item Minimum collateralization ratio upon initialization
  \item Minimum loan size
  \item Which CEXes will be used as price feeds for ADA, DANA and dUSD?
\end{itemize}
