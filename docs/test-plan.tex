...
\todo{It might be best to list what features of the protocol we foresee most 
  frequently updating so we can effectively test-run different kinds of updates.
  The below is proposed for schematic upgrades of the buffer and the vault.}

\subsubsection{Buffer Updates}

\todo{Proposal: The buffer can support several versions at the same time, which a vault
can consume based on it's compatible version.}
...
\todo {
  Proposal: The buffer can expose different versions of it's interface to a vault
  and the vault can determine it's compatibility and update-ability during vault
  operations and execute updates when (explicit update, debt payment, debt issuance)
  are being made. This can be performed by accessing the datum of the buffer when
  passed as a readonly input into a vault transaction.
  Vault updates occur through composition of migration functions e.g v1 -$>$ v2, v2 -$>$ v3. 
} 
